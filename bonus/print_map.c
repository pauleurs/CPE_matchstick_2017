...
void print_map_two_player_1(int player, int size, int size_line, int line)
{
	if (player == 0)
		mvprintw((LINES / 2) + (line - (size / 2)),		\
			(COLS / 2) - (size_line) / 2  - 2, "->");
	else
		mvprintw((LINES / 2) + (line - (size / 2)),		\
			(COLS / 2) + (size_line) / 2  + 2, "<-");
}
...
void print_map_two_player(char **map, int size, int line, int player)
{
	...
	print_map_two_player_1(player, size, size_line, line);
	while (map[i] != NULL) {
		if (i == line) {
			attron(A_DIM);
